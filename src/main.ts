...
async function init(config: AppConfig) {
    ...
    for (const taskConfig of config.tasks) {
        ...
        const client = new HYPClient(launcherId, {
            ...
            sub_channel: taskConfig.launcher.subChannel || defaultChannel.sub_channel
        });
        ...
        for (const game of await client.getGames()) {
            const gameFilter = gameFilterMap.get(game.biz) ?? gameFilterMap.get('*');
            if (gameFilter) {
                taskGames.push({
                    id: game.id,
                    biz: game.biz,
...
async function removeDeprecatedFiles(storage: FileStorage, deprecatedFileList?: BasicFileInfo[]) {
...
async function executeTask(task: AppTask) {
    ...
    const gamePackages = await task.client.getGamePackages(task.games.map((game) => game.id));
    for (const game of task.games) {
        const gamePackage = gamePackages.find((gamePackage) => gamePackage.game.id === game.id);
        if (!gamePackage) {
            console.error('启动器 API 没有返回期望的游戏', game.id, game.biz, game.display.name);
            continue;
        }
        console.log('正在处理游戏', game.id, game.biz, game.display.name);
        game.provider.update(gamePackage);
        promisesRemove.push(removeDeprecatedFiles(task.storage, game.provider.deprecatedFileList));
        promisesSync.push(
            syncStorage(task.storage, game.provider.fileList).then((publicFileInfos) => ({
...
async function sync() {
    try {
        ...
        if (!isEqual(status, publicStatus)) {
            publicStatus = status;
            if (config.statusFile) {
                await fsPromises.writeFile(config.statusFile, JSON.stringify(status));
            }
        }
    } catch (err) {
